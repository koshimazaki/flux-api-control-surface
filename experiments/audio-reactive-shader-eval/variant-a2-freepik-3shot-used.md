# Variant A2 - Freepik 3-Shot Prompt Used

This captures the actual A2 prompt chunks used for the no-audio 3-shot baseline. The platform durations were `4s`, `2s`, and `3s`.
...
- This is the no-audio baseline counterpart to the later B2 audio sequence attempts.
- The platform shot durations are integer chunks, but the text inside the chunks still references the original continuous timeline.
